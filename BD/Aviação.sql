...
-- Inserção de dados na tabela passagem
...
-- Inserção de dados adicionais na tabela aeronave
INSERT INTO aeronave (prefixo, modelo, lugares, autonomia)
VALUES
...
-- Inserção de dados adicionais na tabela piloto
INSERT INTO piloto (registro, nome, telefones, dependentes, endereco)
VALUES
...
-- Inserção de dados adicionais na tabela passageiro
INSERT INTO passageiro (CPF, nome, nascimento, telefone, endereco)
VALUES
...
-- Inserção de dados adicionais na tabela voo
INSERT INTO voo (numero, origem, destino, datahora_saida, aeronave_prefixo, piloto_registro)
VALUES
...
-- Inserção de dados adicionais na tabela passagem
INSERT INTO passagem (bilhete, assento, check_in, voo_numero, passageiro_CPF)
VALUES
...
select * from voo v inner join piloto p on v.piloto_registro = p.registro;

select * from voo v inner join aeronave a on a.lugares = 0;

select nome,cpf from passageiro p inner join voo v;

select * from passageiro p inner join passagem pa on pa.passageiro_CPF = p.cpf order by voo_numero;

SELECT
    numero AS numero_voo,
    origem,
    destino,
    datahora_saida,
...
FROM
    voo v
JOIN
    aeronave a ON v.aeronave_prefixo = a.prefixo;
    
    SELECT
    destino,
    COUNT(pa.bilhete) AS quantidade_passageiros
FROM
    voo v
LEFT JOIN
    passagem pa ON v.numero = pa.voo_numero
GROUP BY
    destino;
